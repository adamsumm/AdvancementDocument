...
%
%* Page 4: You say that Kamak generates rooms "in the Mappy sense." But Kamek focused exclusively on platformers, while Mappy mostly maps non-platformers. In general, there's a bit of confusion in this chapter about how much you're focusing on platformers. Your related work is almost entirely platform level generation, but Agahnim doesn't do platform-level generation (which means you should say something about other kinds of level generators in related work).   --- I discuss Joris's work, van Der Linden, Sorenson and Pasquier, Valtachanov and Brown, Horswill and Foged, Valls-Vargas
%* Page 26: How will Kamek and Agahnim use the output of the new mappy (with mechanical properties)? Kamek wants annotated maps - so will Mappy be outputting maps in this Kamek form? Agahnim doesn't make use of mechanical properties at all, right? So what does it mean to integrate the new Mappy with Agahnim? Separately from mechanical properties, is there any other work that needs to be done to make Mappy output flow seamlessly into these two generators? Somehow I don't quite believe the very sparse description you give here (which makes it sound like it will basically be trivial :) ). 
%* Page 27: It's unclear whether your still going to have two generators at the end, or whether you're basically collapsing Kamek (K) and Agahnim (A). The A Refinements section sounds like K will be embedded in A. But the K Refinements section sounds like you'll be adding latent representation to K (which will then be embedded in A)? Also, it's not clear when you'd use K rather than A for a generation task. Why not use K to generate a Zelda dungeon? Why not use A to generate a Super Mario level? What's the relationship between the capabilities of these two generators? Finally, your discussion of the A Refinements is way to terse to follow. What's a pointer network and what power does it give you? How do the new A internals relate to the capabilities of the old A internals? Does the input representation need to change between the old A and the new A? 
%

%* Page 8: Big gaps between paragraphs.   -- LaTeX's gonna LaTeX

%* Page 28: "Figure ??" --> make sure you fill in.
%

%* Page 35: What do input and output representations look like for CALE? Also, since this is a similar architecture to the new K and A, you definitely need to make sure you talk about pointer networks somewhere and also talk through in more detail what this architecture is doing for you. Again, proposed work section super sparse here. 
%
%* Page 36: Won't you be training CALE against a collection of hybrid automata? So couldn't you use some kind of volume statistics measure like you're proposing for level generation?
%
%* Page 50: Need cites for FOIL, GOLEM, Metagol. 
%
%* Page 52: You should provide examples of what you'd like the system to learn from the Cygnus game description of Pong. You're walking through lots of steps in the abstraction process, but the reader is unclear on where it's going and what the gold-standard knowledge is that you'd like induction to learn for you.
...
\begin{enumerate}
\item For a designer in process, design decisions are rarely accurately accessible via introspection \cite{nisbett1977telling,artistic_practices}
...
This is not to say that it can not be done, as, obviously, people have done this.  But it is to say that in general it is a hard, arduous endeavor that can still lead to feelings of ``oatmealishness'' \cite{kate_oatmeal,mikepcg}, i.e. content that while different in countless different ways, still feels the same.  
...
Moving away from videogames, Cameron Browne's   \textit{Ludi} \cite{browne2011evolutionary} generates 2 player board games.  \textit{Ludi} uses EC to generate games and then undergoes a multi-step evaluation.  Similar to the work of Nielsen \textit{et al.}, simulated players play the games and their playtraces are evaluated for different aesthetic criteria, such as whether the game is balanced (player order does not matter) or the game has drama (a player can come back from a bad position to win the game). While a wide variety of rulesets are possible with the \textit{Ludi} game formalism, the EC tended to favor $N$-in-a-row goals more heavily than other types of rules, perhaps speaking to an issue in evaluation.
