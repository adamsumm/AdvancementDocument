...
There are two major components of this work:

\begin{itemize}
\item Learning from games --- levels, behaviors, higher order properties
\item Generating levels, behaviors, games targeting higher order properties --- from the learned properties
\end{itemize}

, so we need to first situate this work in the context of the existing work.

\subsection{Learning from Games}
...
Recent work by Guzdial et al. \cite{guzdial2017game} learns game state transitions from observation.  Taking in frame-by-frame image locations, they greedily learn rules that best describe the transitions of those images to the next frame.  By operating at the level of frame-to-frame transitions learned from an annotated video stream, a number of complications arise.  Their approach convolves level design and game rules in a way that is both unintuitive to humans and hamstrings their rules, i.e., they must learn all of the levels of a game as rules.  These ``level design rules'' are unlikely to generalize well and very rarely do humans encode a level as a collection of rules.


\subsection{Procedural Generation of Levels}
...
\subsection{Procedural Generation of Mechanics, Behaviors, and Games}
...
\noindent to describe the base definitions of the game, but it is in the rules and interactions that govern these entities and resources that the game is found.
...
\item Whether the distance between two types of entities, given some criterion (nearest, furthest, random, average), is above or below a threshold
\end{itemize}
...
\begin{itemize}
\item Modify resource $A$ (or property $B$ of entity $C$) by either adding or subtracting value $B$ from it
...
\begin{figure*}[h]
\centering
\includegraphics[width=0.45\textwidth]{figures/WORK_TO_BE_DONE.pdf}
\caption{ The work that has been done and that which remains (dotted lines).  Understanding systems are green and generation systems are orange.}
\label{fig:remaining_work}
\end{figure*}


The work that I have done so far falls broadly in two categories:

\begin{enumerate}
\item Game Understanding/Analysis
\item Game Content Generation
\end{enumerate}

Except for the work on \textit{Gemini}, these two categories have  remained cleanly partitioned.  A large part of the remaining work  comes in bridging these different works.  Figure \ref{fig:remaining_work}  shows my existing body of work (anything with a firm outline), and the remaining work (anything with a dotted line).  The largest remaining pieces are:



\begin{itemize}
\item \textit{Leda} --- a system that takes in Cygnus specified games, observations about them, and attempts to deduce the rules required to make those inferences
\item \textit{Mappy} --- Mappy currently produces maps at a fairly low level, namely the tiles that make up Mappy levels are just the set of all seen tiles at that position. To make Mappy useful for downstream generative processes, I intend to incorporate the work on causal affordance learning into Mappy. 
\item \textit{Ingo} --- Ingo represents the next iteration of a graph-based, machine learned level generator.  The earlier Bayes Net approach had a number of drawbacks - a lot of human annotation, poor generalization.  Ingo represent a switch to a neural approach, which will hopefully not have these short-comings.
\item \textit{Talon} --- Talon represents the next iteration of the tile-based, machine learned room generator.  The largest difference between Talon and the earlier work is that Talon will act as an encoder of levels, and not just an auto-regressive process.  This should enable better capabilities in Ingo, and allow for more interesting usage of Talon (e.g., conceptual blending between levels)
\item \textit{Protoman} --- Protoman will generate hybrid automata characters and behaviors, as observed by CHARDA.  The goal of Protoman is to learn from entity behaviors, so as to be able to generate new behaviors.  While most of the previous work has been within one game (e.g., learning levels from just Super Mario Bros.), Protoman will, almost by necessity, have to learn from many different game, due to a lack of variety within most games. Some games allow have meaningfully important character/behavior differences, but most games have a single controlled character and a small set of character behaviors.  To generalize, Protoman will have to look at multiple games and learn the generalizable structure to characters.

\end{itemize}

\subsection{Leda}

Leda is in an early state, but the goal for Leda is to take a Cygnus specified game (as in Castor or as produced by Pollux), and a set of observations about those games (e.g., ``The player controls the paddle'', ``The player's goal is to increase their score'', etc.)
...
\subsubsection{Leda --- Preliminary Results}

Work on \textit{Leda} is still in an early stage; however, \textit{Leda} is able to produce low level inference rules important for higher level inferences.  We tested \textit{Leda} on two games written in Cygnus, \textit{Pong} and \textit{Kaboom!}, over four low level learning tasks, determining why a given entity would be labeled as being controlled by the player, why an entity would be labeled as static, why a given outcome would be labeled as being controlled by the player, and why an entity would be labeled as being controlled by the computer. The rules that \textit{Leda} learned are:

\noindent \texttt{player\_controls(X) :- }
 
\texttt{precondition(control\_event(C),O),}

\texttt{result(O,moves(X,S,D)).}

i.e. ``The player controls X if there is a precondition predicated on a control event that results in that entity moving in some way.''


\noindent \texttt{non\_static(X) :- }

\texttt{result(O,moves(X,S,D)).}

i.e. ``An entity is non-static if it moves in some way.''

While \textit{Leda} can not currently perform predicate invention, by utilizing previously learned rules it can learn rules such as:


\noindent \texttt{player\_controls\_outcome(O) :- }

\texttt{precondition(control\_event(C),O).}

\noindent \texttt{player\_controls\_outcome(O) :- }

\texttt{precondition(overlaps(X,Y),O),}

\texttt{player\_controls(X).}

i.e. ``The player controls outcome O if either that outcome is predicated on a control event or collision involving an entity X that the player controls.''


\noindent \texttt{computer\_controls(X) :- }
 
\texttt{result(O,moves(X,S,D),}

\texttt{not player\_controls(X).}

``The computer controls entity X if it moves in some way and the player does not control X.''

While these rules are all quite simple, they are the building blocks for higher order inferences, and it points towards promising future directions for \textit{Leda}.  

\subsubsection{Leda --- next steps}

The next steps for Leda are:

\begin{itemize}
\item Consider multiple rules at a time --- currently, Leda only generates single rules at a time, but there are many types of rules that require an \textit{or} to fully cover the given observations (e.g., current Cygnus specifications for clicking-and-dragging do not rely on \texttt{precondition(control\_event(C),O)}, so would not be covered by the single rule above).  
\item Use ASP language constructs --- Leda is designed to interface with Castor and Pollux, two ASP programs, and the rules they use often contain AnsProlog refinements on the standard Prolog currently used by Leda (e.g., counting such as \texttt{\{p(A)\}})
\end{itemize}

\subsection{Mappy}

Currently, Mappy does not capture mechanically meaningful interactions, or rather it stores them but does not process them in a meaningful way.  For instance, while the full track of Mario is captured, the fact that Mario is ``interacting'' with entities in the game --- jumping on Goombas, hitting $?$ blocks --- is not incorporated.  Furthermore, important state interactions --- a $?$ block turning into an inert block after being hit on the underside by Mario --- are completely glossed over as just a series of frames with no context (e.g., frame 0 - tile $(4,5)$ is a $?$ block, frame 60 - tile $(4,5)$ is blue sky, frame 120 - tile $(4,5)$ is an inert block).  

The work that I did on causal affordance learning is a natural fit here.  It is less an open research question as to whether it can be used here, but more an engineering step required to further refine Mappy.

\subsection{Talon}

The key reason for modifying Mappy is to support Talon in a way that does not rely on human annotated levels.  Previously, Talon used levels annotated at a broad mechanical level (e.g., inert solid tiles were one type, question mark blocks another, enemies another, etc.), so it is an open question as to how to best incorporate Mappy produced maps.   Each tile has a visual signifier and its mechanical property clusters (either single membership or multiple membership), so the possible ways of representing are:

\begin{itemize}
\item Visual Signifier --- Possibilities are: No visual information included, first seen visual, modal seen visual, set of all seen visuals
\item Mechanical Properties --- Possibilities are: No mechanical properties included, single membership cluster identity, multiple membership cluster identities
\end{itemize}

These 11 different combinations ($4$ visual options $\times$ $3$ mechanical options, with the ``no visuals, no mechanics'' option be excluded since it would include no information) should then be compared to the earlier human annotated generators, to see what data is most important for a machine learned generator.

Finally, I would like to encode the levels in a lower dimensional space.  Recently, there has been a lot of focus on Generative Adversarial Networks (GANs) \cite{gans}, Variational Autoencoders \cite{vae}, and sequence-to-sequence models \cite{seq2seq} (which I used for \textit{Mystical Tutor}) which all operate with a fixed dimensional latent ``encoding'' of the content they generate.  Some, such as the GAN, attempt to learn this by producing output in an attempt to confuse a critic network as to what is generated and what is an actual representative. Others, like the VAE and sequence-to-sequence models learn an encoded latent representation that is then decoded to reproduce the input.  The advantage to approaches such as these, is that they tend to learn densely informative latent representations, that can be utilized for meaningful modifications.  For instance, some work has shown that head angle, lighting angle, and facial gesture \cite{ganstuff} can be learned as dimensions in the latent space, allowing users to seamlessly change images by modifying these latent parameters.  Similarly, interpolating in the lower dimensional space is capable of providing conceptually blended artifacts in a way not possible by averaging pixel values between images \cite{ganinterpolation}.  Toward this end, I would like to learn a level representation that is capable of being utilized in these manners, with the goal being the discovery of important latent features (e.g., difficulty, etc.) that a user can modify and a meaningful generative space to be sampled from.  This meaningful space is an important component for future work on Ingo, which I will now discuss.

\subsection{Ingo}

Previously, Ingo used Bayes Nets to generate levels structured as graphs; however, it relied heavily on human annotation, both of the levels, and of the important mechanical properties found in the levels.  Furthermore, the Bayes Net approach had problems with generalization due to the tabular nature of its learned representation.  In an effort to alleviate these problems and to better interoperate with Talon, I intend to move from a Bayes Net approach to a neural approach.  The proposed architecture for Ingo can be seen in figure \ref{fig:Ingo}.
...
\subsection{Procedural Generation of Games}
